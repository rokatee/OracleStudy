...
-- 그리고 이들 각각의 테이블들 간의 관게를 설정하여 연결시켜 놓은 구조
...
※ ORACLE 은 숫자 표현 타입 한 가지로 통일되어 있다
   1. 숫자형 
...
※ ORACLE 의 문자 표현 타입
   ...
   2. 문자형 
             ...
             CHAR(10)     ←← '실습박명수'   10Byte 딱 들어간다 
             CHAR(10)     ←← '실습박명수다' 10Byte 초과하므로 입력 불가
             
             
             VARCHAR2       → 가변형 크기
                               (상황에 따라 크기가 변경)
             VARCHAR2(10) ←← '유재석'       6Byte 이면서 6Byte 를 소모
             VARCHAR2(10) ←← '실습박명수'   10Byte 딱 들어간다
             VARCHAR2(10) ←← '실습박명수다' 10Byte 초과하므로 입력 불가
...
SELECT 사원번호, 사원명, 직종명, 급여, 부서번호
FROM EMP
WHERE 부서번호가 20번과 30번 직원들의 데이터들 중;
...
--==>> 부서번호가 20이면서 30인 것은 없으므로 AND 가 아닌 OR 가 맞다
